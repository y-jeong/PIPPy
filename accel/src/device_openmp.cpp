...
//  dvdr_partial = (double*) dev_malloc(npairs*grid_size*sizeof(double));

//  v_partial = (double*) dev_malloc_host(sizeof(double));
//  dvdr_partial = (double*) dev_malloc_host(npairs*sizeof(double));
...
#pragma omp target teams distribute parallel for map(tofrom:v) reduction(+: v,dvdr[:npairs])     
      for(int i=0; i<nterms; ++i) {
        ...
        for(int j=0; j<npairs; ++j) dvdr[j] -= arg * d_ind[i*npairs+j];
