...
contains
    ...
    subroutine magma_zsetvector( &
        n, hx_src, incx, dy_dst, incy, queue )
        use iso_c_binding
        integer(c_int),            value  :: n, incx, incy
        complex(c_double_complex), target :: hx_src(n)
        type(c_ptr),               value  :: dy_dst
        type(c_ptr),               value  :: queue
        
        call magma_setvector_internal( &
                n, int(sizeof_complex16), c_loc(hx_src), incx, dy_dst, incy, queue, &
                "magma_zsetvector" // c_null_char, &
                __FILE__ // c_null_char, &
                __LINE__ )
    end subroutine

    subroutine magma_zgetvector( &
        n, dx_src, incx, hy_dst, incy, queue )
        use iso_c_binding
        integer(c_int),            value  :: n, incx, incy
        complex(c_double_complex), target :: hy_dst(n)
        type(c_ptr),               value  :: dx_src
        type(c_ptr),               value  :: queue
        
        call magma_getvector_internal( &
                n, int(sizeof_complex16), dx_src, incx, c_loc(hy_dst), incy, queue, &
                "magma_zgetvector" // c_null_char, &
                __FILE__ // c_null_char, &
                __LINE__ )
    end subroutine
