...
integer :: ntarget = 100       ! End when you reach ntarget basis functions

!integer, dimension(:), allocatable :: ibasis  ! = p(k), permute group
!integer, dimension(:,:), allocatable :: ind   ! = greek-xi(k,j), exponents
...
! j is used to index atom pairs w/ max npair read from basis.dat
...
double precision, dimension(:,:), allocatable :: covu,covsav ! temp cov. matrices(k1,k2)
...
!allocate(ind(nterm,npair))
!read(10,*)
!do i=1,nterm
!read(10,*)k,ibasis(k),dum,(ind(k,j),j=1,npair)
!enddo
close(10)

! read the B matrix (maybe change to binary, as this matrix can be large)
open(20,file="xmat.50000")
read(20,*)
ndat=0
do
  read(20,*,end = 20)
...
xty(k)=0.d0
do i=1,ndat
xty(k)=xty(k)+bb(i,list(k))*vv(i) ! X^T in eq 5
enddo
enddo
lwork=nopt
do k1=1,nopt
...
enddo
enddo
call dgetrf(nopt,nopt,cov,nopt,ipiv,info)           ! invert it using LAPACK
call dgetri(nopt,cov,nopt,ipiv,work,lwork,info)
do k1=1,nopt
cc(k1)=0.d0
do k2=1,nopt
cc(k1)=cc(k1)+cov(k1,k2)*xty(k2) ! c in eq 5
...
enddo
err=dsqrt(err/wrr)
write(6,123)nopt,err,1,vv1,vv2,vf1,vf2





! eventually, we want to implement backward steps, too
! then we want some flexibility to go N steps forward, then M steps back, etc...
! for now we just to forward
...
xty(nopt+1)=0.d0
do i=1,ndat
xty(nopt+1)=xty(nopt+1)+bb(i,ii)*vv(i)
enddo

do k1=1,nopt+1
cc(k1)=0.d0
do k2=1,nopt+1
cc(k1)=cc(k1)+covu(k1,k2)*xty(k2)
enddo
enddo
...
do k1=1,nopt
do k2=1,nopt
cov(k1,k2)=covsav(k1,k2) ! new cov matrix for the growing basis
enddo
enddo
xty(nopt)=0.d0
do i=1,ndat
xty(nopt)=xty(nopt)+bb(i,iisav)*vv(i) ! new X^T*y for the growing basis
enddo
