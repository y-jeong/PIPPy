...
c read
      ...
      IF (my_id.eq.0) THEN
      read(7,*)natom
      read(7,*)(symb(k),k=1,natom)
      read(7,*)(iagroup(k),k=1,natom)
      read(7,*)lreadbasis,ipow,ipowt,imode
      write(6,'(100("*"))')
      write(6,*)
      if (lreadbasis) then
...
c       linter = use intermolecular distances only?
     ...
     & write(6,'(1x,a,i10,a,/)')"Distributing job over ",nproc," cores"
      ...
      IF (my_id.eq.0) THEN
        ...
        do i=1,nchan
          read(7,*)(changroup(i,k),k=1,natom)
        enddo
      ENDIF
...
#ifdef _USE_RESTART
      inquire(file='restart_pippy.in', EXIST=found_restart)
      if(found_restart) then
      
        open(unit=20, file='restart_pippy.in')
        do i=1, nterm
          read(20,*) j,ifailmpi(i),ibmpi(i)
        end do
        close(20)
         ...
         if(my_id == 0) then
            write(*,*) 'nterm= ',nterm,' npermute= ',npermute,
     &       ' npairs= ',npairs
            call flush(6)
         end if
         ...
         allocate( ind_i(npairs, nterm) )
         allocate( ind_ii(npairs) )
...
ccccc MPI DO LOOP
      DO ii=my_id+1,nterm,nproc
         ...
         if (mod(ii,100).eq.0) then
            call cpu_time(tstop)
            nt = nt + 1
            write(6,'(a,i5,a,i6,a,i3,a,f12.6,a,f12.6,a)') "  step ",ii,
     &       " of ",
     &       nterm," on proc ",my_id,' time= ', tstop-tstart,
     &       ' seconds (',(tstop-tstart)/nt," per)"
            call flush(6)
         endif
         ...
         match = 0
         do i=ii-1,1,-1
            if ( (indord(ii).eq.indord(i)) .and.
     &          (ifailmpi(i).eq.0) ) then
               
#if 1
               call search_quick(match, ii, i, npermute, npairs, nterm,
     &          ind_i, ind, ind_ii, ix)
#else          
               call search_full(match, ii, i, npermute,
     &          npairs, nterm, ind_i, ind, ix)
#endif          
               if(match == 1) goto 1010
...
#ifdef _USE_RESTART
      if(my_id ==0) then
        open(unit=20, file='restart_pippy.out')
        do i=1, nterm
          write(20,*) i,ifailmpi(i),ibmpi(i)
        end do
        close(20)
      endif
#endif
...
c     remove unconnected and intramolecular only terms if required
     ...
     & " basis.dat"
      ...
      a=natom
      if (linter) a=natom1
      !basisinfo=" ! atoms, atom pairs, groups/coefficients, terms"
      !write(55,'(4i12,A)')a,npairs,nncoef,nterm,basisinfo
      write(55,'(4i12$)')a,npairs,nncoef,nterm
      write(55,'(A)')" ! atoms, atom pairs, groups/coefficients, terms"
...
***************************************************
       ...
       subroutine search_quick(match, ii, i, npermute, npairs, nterm,
     &  ind_i, ind, ind_ii, ix)
         ...
         integer, intent(in) :: ii, i, npermute, npairs, nterm
         integer, intent(in) :: ind_i(npairs, nterm)
         integer, intent(in) :: ind_ii(npairs)

         integer, intent(in) :: ind(maxterm, maxpair)
         integer, intent(in) :: ix(maxperm, maxpair)

         integer :: j,k, l, m

         integer :: count_nonzero
         integer :: list_nonzero(npairs) ! lazy, static allocation

         ! -- take advantage of ind being sparse (e.g. 98% zeroes)
         
         ! -- loop over elements of ind_i looking for nonzero entries ensuring we find at least one

         count_nonzero = 0
         do k=1, npairs
            if(ind_i(k,i) .ne. 0) then
               count_nonzero = count_nonzero + 1
               list_nonzero(count_nonzero) = k
            endif
         end do

         if(count_nonzero == 0) then
            write(*,*) 'Error search_quick() :: count_nonzero is zero'
            stop
         endif

         ! -- loop over subset of rows looking for columns with matching nonzero elements

         do l=1, count_nonzero
            k = list_nonzero(l)
         
            do j=1,npermute ! this could be replaced with a sorted ind maybe

               if(ind_i(k,i) == ind_ii(ix(j,k))) then

                  ! -- matching elements found, compare both columns
                  
                  match = 1
                  do m=1, npairs
                     if(ind_i(m,i).ne.ind_ii(ix(j,m))) then
                        match = 0
                        exit
                     end if
                  end do
                  ...
                  if (match .eq. 1) goto 1030
            
               endif ! if(element in two columns match)
            
            enddo ! loop over npermute
         enddo ! loop over nonzero rows
         
1030     continue
         
       end subroutine search_quick
       
***************************************************

       subroutine search_full(match, ii, i, npermute, npairs, nterm,
     &  ind_i, ind, ix)
         ...
         do j=1,npermute
            ...
            do k=1,npairs
               if (ind_i(k,i).ne.ind(ii,ix(j,k))) then
                  match = 0
                  exit
               endif
            enddo
            ...
            if (match .eq. 1) then
               if(ii < 100) then
                  write(*,*) 'match found: ii, i, j= ', ii,i,j
               endif
               go to 1020
            endif
