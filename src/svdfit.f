...
c        if (i.eq.2) print *,i,y(i)
   ...
   12  enddo
#ifdef _USE_LAPACK
       write(6,*) 'Using dgesvd()'
       lwork = -1
       info = 0
       allocate(work(1))
       call dgesvd('O', 'A', ndata, ma, u, mp, w, u, mp, vt, np, work,
     &  lwork, info)
       lwork = work(1)
       deallocate(work)
       allocate(work(lwork))
...
#endif                                                   ! Edit the singular values, given TOL from the
       wmax=0.                                           ! parameter statement, between here ...
       do 13 j=1,ma
